...
class MapServer(object):
    ...
    def __init__(self, world):
        ...
        # handler.
        b = rospy.Service('get_bounds', GetBounds, self._get_bounds)
        # TODO: Create a rospy service called 'get_score'. Make _get_score the
        # handler.
        s = rospy.Service('get_score', GetScore, self._get_score)
    ...
    def _get_score(self, request):
        ...
        if request.row > self._world.num_rows() or request.col > self._world.num_cols():
            raise rospy.ServiceException('Location out of bounds')
